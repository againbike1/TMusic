   ...
   rightButtonTitle:(NSString *)rigthTitle
{
    if (self = [super init]) {
        ...
        self.alertContenTextField = [[UITextField alloc] initWithFrame:CGRectMake((kAlertWidth - contentLabelWidth) * 0.5, CGRectGetMaxY(self.alertTitleLabel.frame), contentLabelWidth, 60)];
        self.alertContenTextField.textAlignment = self.alertTitleLabel.textAlignment = NSTextAlignmentCenter;
        self.alertContenTextField.textColor = [UIColor colorWithRed:127.0/255.0 green:127.0/255.0 blue:127.0/255.0 alpha:1];
          self.alertContenTextField.clearButtonMode = UITextFieldViewModeWhileEditing;
        
        self.alertContenTextField.font = [UIFont systemFontOfSize:15.0f];
        [self addSubview:self.alertContenTextField];
             [self.alertContenTextField becomeFirstResponder];
        CGRect leftBtnFrame;
...
#define kButtonBottomOffset 10.0f
        ...
        self.alertContenTextField.placeholder = content;
...
- (void)left
{
    _leftLeave = YES;
    [self dismissAlert];
    if ([self.delegate respondsToSelector:@selector(dxWithTextField:)]) {
        [self.delegate dxWithTextField:self.alertContenTextField.text];
    }
}
...
- (void)removeFromSuperview
{
    ...
    [UIView animateWithDuration:0.15f delay:0.1 options:UIViewAnimationOptionCurveEaseOut animations:^{
