...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _currentWaterColor = [UIColor colorWithRed:127/255.0f green:127/255.0f blue:127/255.0f alpha:0.5];
        _currentLinePointY = 25;
...
- (void)drawRect:(CGRect)rect
{
    ...
    for(float x=0;x<=SCREEN_W;x++){
        y= a * sin( x/180*M_PI + 2*b/M_PI ) * 2.5 + _currentLinePointY;
        CGPathAddLineToPoint(path, nil, x, y);
    }
    
    CGPathAddLineToPoint(path, nil, 375, rect.size.height);
